...
architecture Behavioral of ethernet_receive is
    ...
    component CC_PLL is
        ...
        port (
            CLK_REF : in std_logic; CLK_FEEDBACK : in std_logic; USR_CLK_REF : in std_logic;
    ...
    begin
        if clk20'event and clk20 = '1' then
            ...
            case v_addr_int is
                ...
                when 50 to 67 => pkt_data <= std_logic_vector(to_unsigned(v_addr_int - 50, 8));
                when others => pkt_data <= x"00";
            end case;

            -- Serialization Control
            if ShiftCount = 15 then v_readram := '1'; else v_readram := '0'; end if;

            if StartSending = '1' then
                SendingPacket <= '1';
            elsif ShiftCount = 14 and v_addr_int = 72 then
                SendingPacket <= '0';
            end if;
            ...
            elsif v_readram = '1' and v_addr_int = 68 then
                CRCflush <= '1';
            end if;

            if v_readram = '1' then
                if v_addr_int = 7 then CRCinit <= '1'; else CRCinit <= '0'; end if;
            end if;
